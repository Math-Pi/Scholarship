  ...
  loginSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        getUserByUserName(values.username,(data)=>{
          if(data==null){
            this.props.form.setFields({
              username: {
        ...
        <Form onSubmit={this.loginSubmit}>
          ...
          <PromptBox info={getFieldError('verification')} width={calculateWidth(getFieldError('verification'))}/>}>
            {getFieldDecorator('verification', {
              validateFirst: true,  //validateFirst：true 表示，antd form按rules顺序进行校验。
              rules: [
          ...
          {/* <Form.Item>
            <Radio.Group onChange={this.onChange} value={this.state.value}>
              <Radio className='radio' value={1}>学生</Radio>
              <Radio className='radio' value={2}>辅导员</Radio>
              <Radio className='radio' value={3}>学工处</Radio>
              <Radio className='radio' value={4}>管理员</Radio>
            </Radio.Group>
          </Form.Item> */}
