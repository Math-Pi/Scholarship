...
export default class Library extends React.Component {
  ...
  handleDelete=(record)=>{
    ...
    Modal.confirm({
      ...
      content: '确认要删除吗？',
      onOk() {
        del([record.gzid], authority.delete, () => {
          _this.indexQuery()
        })
      }
    })
      ...
      <div>
        ...
        <Modal title={this.state.title} width={600} visible={this.state.addFlag} onOk={this.handleSubmit} onCancel={this.handleCancel}>
          <Form ref={ele=>this.addMadoal=ele} onSubmit={this.handleSubmit}>
            <Form.Item {...formItemLayout} style={{ marginBottom: 0 }} label="规则编号">
              {getFieldDecorator('gzbh', {
                initialValue: this.state.selectRow.gzbh,
                rules: [
                  {
                    required: true,
                    message: '必填',
            ...
            <Form.Item {...formItemLayout} style={{ marginBottom: 0 }} label="规则名称">
              {getFieldDecorator('gzmc', {
                initialValue: this.state.selectRow.gzmc,
                rules: [
                  {
                    required: true,
                    message: '必填',
