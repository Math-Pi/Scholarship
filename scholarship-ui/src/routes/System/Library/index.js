...
import { query,update,create,del,queryRule } from './service'
      ...
      },
      {
        ...
        render: (text, record) => (
          <div>
            ...
            <a onClick={()=>this.handleDelete(record.dmid)}>删除</a>
          </div>
        ),
      }
    ]
    
    const {gzmc,dmk,dmv}=this.state.selectRow
    const {pageNo,pageSize,total}=this.params
    return (
      <div>
        <CustomBreadcrumb arr={['代码库']} />
        ...
        <Modal title={this.state.title} width={600} visible={this.state.addFlag} onOk={this.handleSubmit} onCancel={this.handleCancel}>
          <Form ref={ele=>this.addMadoal=ele} onSubmit={this.handleSubmit}>
            <Form.Item {...formItemLayout} style={{ marginBottom: 0 }} label="规则名称">
              {getFieldDecorator('gzmc', {
                initialValue: gzmc,
                rules: [
                  {
                    required: true,
                ...
                <Select placeholder="请选择" disabled={!!gzmc}>
                  {this.state.ruleList.map((val)=>(
                    <Select.Option key={val.gzbh} value={val.gzbh}>{val.gzmc}</Select.Option>
                  ))}
                </Select>
              )}
            </Form.Item>
            <Form.Item {...formItemLayout} style={{ marginBottom: 0 }} label="代码">
              {getFieldDecorator('dmk', {
                initialValue: dmk,
                rules: [
                  {
                    required: true,
            ...
            <Form.Item {...formItemLayout} style={{ marginBottom: 0 }} label="代码值">
              {getFieldDecorator('dmv', {
                initialValue: dmv,
                rules: [
                  {
                    required: true,
